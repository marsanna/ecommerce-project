...
export const orderItemInputSchema = z.strictObject({
  ...
  title: z.string({ error: "Product title must be a string" }).min(2, {
    message: "Product title is required and must be at least 2 characters long",
  }),
  price: z
...
export const orderOutputSchema = orderInputSchema.extend({
  user: z.string().min(1, { message: "User name must not be empty" }),
  orderId: z
    .string()
    .refine((val) => isValidObjectId(val), { message: "Not a valid order id" }),
});
